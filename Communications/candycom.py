...
class ClientComms:
    def __init__(self, board_config, comm_mode="serial", buffer_size=64, custom_commands=None):
...
#---------------------------------------------------------------------------------------------------#
    ...
    async def mesage_interpreter(self): # pull a message from the buffer and figure out what it means
        ...
        elif type(self.custom_commands) is not None:
            if message in self.custom_commands:
                await self.custom_commands[message]()
